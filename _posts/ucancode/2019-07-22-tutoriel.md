...
Ce tutoriel vous fait découvrir les plateformes utilisées pour les activités de programmation [UCAnCODE](http://i3s.unice.fr/master-info/etus/ucancode/). Les objectifs principaux de ces activités sont les suivants :
...
Le tutoriel est ouvert à tous. Aucune inscription n&rsquo;est nécessaire.
...
Des solutions dans plusieurs langages sont disponibles dans ce [dépôt github](https://github.com/arnaud-m/echo) ou directement ci-dessous.
...
Téléchargez un de ces programmes dans un répertoire de travail créé pour l&rsquo;occasion.
...
Le contenu du fichier `ECHO-03.in` va être redirigé sur l&rsquo;entrée standard pour être lu par notre programme. Notre programme va écrire sur la sortie standard.
...
L&rsquo;opérateur de redirection `<` n&rsquo;existe pas encore dans powershell. Il faut donc utiliser une commande équivalente combinant la commande `cat` et un tube (*pipe* en anglais).
